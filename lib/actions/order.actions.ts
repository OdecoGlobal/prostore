...
export async function createPayPalOrder(orderId: string) {
  try {
    ...
    if (order) {
      ...
      //update order wit paypal order id
...
export async function approvePayPalOrder(
  ...
  try {
    ...
    return { success: true, message: 'Your order has been paid for' };
  } catch (error) {
    return { success: false, message: formatError(error) };
  }
}
...
async function updateOrderToPaid({
  ...
  await prisma.$transaction(async tx => {
    // Iterate over products and update the stick
    for (const items of order.orderitems) {
      await tx.product.update({
        where: { id: items.productId },
        data: { stock: { increment: -items.qty } },
      });
    }
...
// Update COD order to deleviered
